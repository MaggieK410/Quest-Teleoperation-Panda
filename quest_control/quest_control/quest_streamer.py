...
class QuestTrajectoryPublisher(TrajectoryPublisherBase):
    def __init__(self):
        ...
        self.agimus_franka_gripper_client = FrankaGripperClient(
            self, arm_id="fer"
        )  # might have to be chnaged to fr3?
        ...
        self.right_camera_path = self.declare_parameter(
            "right_camera", "/dev/video6"
        ).get_parameter_value()
        self.left_camera_path = self.declare_parameter(
            "left_camera", "/dev/video8"
        ).get_parameter_value()
        ...
        self._marker_base = Marker(
            ...
            color=ColorRGBA(**dict(zip("rgba", [1.0, 0.0, 0.0, 0.5]))),
            lifetime=Duration(seconds=0.01 * 10.0).to_msg(),
        )
        self.get_logger().info(
            "Quest trajectory publisher started, waiting for Quest data..."
        )

    def publish_controller_tf(
        self, T, parent_frame="world", child_frame="quest_right_controller"
    ):
        t = TransformStamped()
    ...
    def _control_callback(self, msg: Control) -> None:
        ...
        try:
            self.latest_torques = np.array(
                msg.feedforward.data, dtype=np.float64
            ).ravel()[:7]
        except Exception:
            pass
    ...
    def ready_callback(self):
        print(
            f"[quest_streamer] ready_callback: robot ready, q0={[round(v, 3) for v in self.q0]}",
            flush=True,
        )

        # Start streaming only once everything is ready
        self.timer = self.create_timer(
            0.01, self.stream_quest_to_robot, callback_group=self.control_group
        )
        self.gripper_timer = self.create_timer(
            0.01, self.gripper_control_robot, callback_group=self.control_group
        )
        self.data_collection_timer = self.create_timer(
            0.05, self.collect_data, callback_group=self.data_group
        )
    ...
    def collect_data(self):
        now = time.time()
        if (
            not hasattr(self, "_last_status_print")
            or now - self._last_status_print > 5.0
        ):
            if self.latest_data is None:
                print(
                    "[collect_data] Waiting for VR data (UDP port 5000)...", flush=True
                )
            else:
    ...
    def _build_state_dict(self) -> "dict | None":
        ...
        try:
            ...
            quest_ref_se3 = (
                pin.SE3(self.quest_ref) if hasattr(self, "quest_ref") else ee_se3
            )
            target_pos = quest_ref_se3.translation.copy()
            target_quat = pin.Quaternion(quest_ref_se3.rotation).coeffs()

            gripper_cmd = (
                0.0 if self.current_gripper_status else 1.0
            )  # 0=fermé, 1=ouvert
    ...
    def send_new_gripper_goal(self, close: bool) -> bool:
        if close:
            ...
            return self.agimus_franka_gripper_client.send_goal(
                position=0.039, max_effort=10.0
            )
    ...
    def decode_packet(self, data: bytes) -> VRFrame:
        ...
        left_index, right_index, left_grip, right_grip = (
            next(f),
            next(f),
            next(f),
            next(f),
    ...
    def stream_quest_to_robot(self):
        ...
        if self.latest_data != None:  # try:
            ...
            if not hasattr(self, "initialized"):
                ...
                pin.forwardKinematics(
                    self.robot_models.robot_model, data, self.current_q
                )
                pin.updateFramePlacements(self.robot_models.robot_model, data)
                ...
                self.last_yaw = np.arctan2(
                    T_hand_in_head[:3, :3][1, 0], T_hand_in_head[:3, :3][0, 0]
                )
        ...
        if self.rotation:
            ...
            yaw_prev = np.arctan2(
                self.last_hand_in_head[2, 0], self.last_hand_in_head[0, 0]
            )
        ...
        for i in range(len(self.buffer)):
            ...
            traj_weights = TrajectoryPointWeights(
                w_robot_configuration=np.array(
                    [15.0, 0.1, 0.06, 0.06, 0.06, 0.06, 0.06]
                ),  # dtype=float), #self.w_q,
                ...
                w_end_effector_poses={
                    self.ee_frame_name: [40.0, 40.0, 40.0, 30.0, 30.0, 30.0]
                },  # self.w_pose
                # w_end_effector_poses={self.ee_frame_name: [3.0, 3.0, 3.0, 1.0, 1.0, 1.0] },
            )
            weighted_traj_point = WeightedTrajectoryPoint(
